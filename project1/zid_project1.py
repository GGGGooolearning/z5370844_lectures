...
def read_dat(tic):
    ...
    data_dat = []
    with open(pth, mode='r') as fobj:
        each_line = fobj.readlines()
        data_dat = [line.strip() for line in each_line]
    #print(data_dat)
    return data_dat
...
def line_to_dict(line):
    ...
    for key, value in COLWIDTHS.items():
        value_widths.append(value)
    #print(value_widths)

    start_end = 0
    for pos in value_widths:
        context_dat.append(line[start_end:start_end+pos])
        start_end += pos
    #print(context_dat)

    dat_split = dict(zip(COLUMNS, context_dat))
    return dat_split
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    #print(tickers_lst)
    turn_list = list(tic_exchange_dic.keys())
    if tickers_lst is None:
        pass
    elif tickers_lst == []:
        raise Exception('error: Empty List')
    else:
        for element in tickers_lst:
            if element in turn_list:
                continue
            else:
                raise Exception(f'{element} is not a key of tic_exchange_dic.')
    ...
    if col_lst is None:
        ...
        for element in col_lst:
            if element in COLUMNS:
                ...
                raise Exception(f"{element} is not found in 'COLUMNS'")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    # print(tic_exchange_dic)
    dict1 = {}
    # {<tic> : <data>}, <tic> = key_tic
    for key_tic in tickers_lst:
        dict2_value = {}
        # <data> = {'exchange':..., 'data':....}
        dict2_value['exchange'] = tic_exchange_dic[key_tic] # "NASDAQ"="AAPL" {'aapl': 'nasdaq'}

        list_data = []
        # {<tic>, {'exchange', list_data[]}}
        lines = read_dat(key_tic)
        # print(lines) list of dat line
        for line in lines:
            dict3 = {}
            # {'Date': ..., 'Close': ....}
            # {<tic>, {'exchange', list_data[{'Date': ..., 'Close': ....}]}}
            for element in col_lst:
                # print(element)
                dict3[element] = line_to_dict(line)[element] # dict[key] get value
            list_data.append(dict3)

        dict2_value['data'] = list_data
        dict1[key_tic] = dict2_value
...
if __name__ == "__main__":
    ...
    # verify_tickers(get_tics(TICPATH),['aaa'])
    # verify_cols([])
    # verify_cols(['aaa'])
    pass
